...
from pptracking.python.mot.utils import flow_statistic, update_object_info, flow_statistic_child
...
class PipePredictor(object):
    ...
    def predict_video(self, video_file, thread_idx=0):
        ...
        while (frame_count!=0):
            ...
            success, frame_rbg = capture.read()
            frame_count-=1
            frame_rgb = cv2.cvtColor(frame_rbg, cv2.COLOR_BGR2RGB)
            ...
            if self.modebase["idbased"] or self.modebase["skeletonbased"]:
                ...
                # if frame_id % 100 == 0: print(mot_res)
                if frame_id > self.warmup_frame:
                ...
                #     # 将当前列表转换为字符串，并用逗号分隔每个元素
                #     for sublist in boxes[0]:
                #         line = ', '.join(map(str, [int(num) for num in sublist]))
                #         # 写入转换后的字符串到文件中，并添加换行符
                #         f.write(line + ';')
                #     f.write('\n')
                #     line = ', '.join(map(str, [num for num in scores[0]]))
                #     f.write(line +'\n')
                #     line = ', '.join(map(str, [num for num in ids[0]]))
                #     f.write(line +'\n')
                ...
                if self.child == False:
                    ...
                    statistic = flow_statistic_child(
                        mot_result,
                        self.secs_interval,
                        self.do_entrance_counting,
                        self.do_break_in_counting,
                ...
                # print(statistic['in_id_list'], statistic['out_id_list'])
                seq_inlist.append(len(statistic['in_id_list']))
                seq_outlist.append(len(statistic['out_id_list']))
        ...
        # for i in range(len(seq_inlist)):
        #     print("seq_inlist:",seq_inlist[i])
        #     print("seq_outlist:",seq_outlist[i])

            import xlwt
            f = xlwt.Workbook('encoding = utf-8')  # 设置工作簿编码
            sheet1 = f.add_sheet('sheet1', cell_overwrite_ok=True)  # 创建sheet工作表
            for i in range(len(seq_inlist)):
                sheet1.write(i, 0, seq_inlist[i])  # 写入数据参数对应 行, 列, 值
                sheet1.write(i, 1, seq_outlist[i])  # 写入数据参数对应 行, 列, 值
